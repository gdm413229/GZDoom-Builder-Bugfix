...
namespace CodeImp.DoomBuilder.PlatformSpecific.Windows
{
    internal static class FileLockCheckerWindows
    {
        ...
        private static FileLockChecker.FileLockCheckResult CheckFileWindowsXP(string path)
        {
            bool locked = false;
            ...
            return new FileLockChecker.FileLockCheckResult { Error = (locked ? "Unable to save the map. Map file is locked by another process." : string.Empty) };
        }

        private static FileLockChecker.FileLockCheckResult CheckFileWindows(string path)
        {
            //mxd. Needs Vista or newer...
            uint handle;
            string key = Guid.NewGuid().ToString();
            FileLockChecker.FileLockCheckResult result = new FileLockChecker.FileLockCheckResult(); //mxd
